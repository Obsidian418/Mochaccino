...
class MoccType {
  ...
  @override
  String toString() {
    if (typeArgs != null) return typeArgs!.map((MoccType t) => "<${t.toString()}>").toList().join('');
    return lexeme;
  }
}
...
abstract class PrimitiveType implements MoccType {
  ...
  static MoccType fn(MoccType returnType) => MoccType('fn', [
        returnType
      ]);
  static MoccType map(MoccType keyType, MoccType, valueType) => MoccType('map', [
        keyType,
        valueType
      ]);
  static MoccType arr(MoccType elementType) => MoccType('arr', [
        elementType
      ]);
  static MoccType Void() => const MoccType('void');
  static MoccType promise(MoccType innerValueType) => MoccType('promise', [
        innerValueType
      ]);
