...
enum ComparativeSymbol { lessThan, greaterThan, lessThanEqual, greaterThanEqual }
  ...
  Expression parseValue() {
    ...
    throw ParseException.raise(
      Issue(
        ...
        description: "Expression expected, but token '${peek().lexeme}' was found. Token details:".newline(peek().toString().indent(4)),
      ),
    );
  }
...
class InitialiserStmt extends Statement {
  ...
  String toTree(int indent) {
    return "${this.runtimeType}"
        ...
        .newline(
          initialiser != null ? initialiser!.toTree(indent + 4) : Value(null).toTree(indent + 4),
        );
  }
}
...
class EqualityOp implements Operator {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: ${symbol.toString().replaceAll('EqualitySymbol.', '')}".indent(indent);
  }
...
class Group implements Expression {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}".indent(indent).newline(expression.toTree(indent + 2));
  }
...
class ArithmeticOp implements Operator {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: ${symbol.toString().replaceAll('ArithmeticSymbol.', '')}".indent(indent);
  }
...
class ComparativeOp implements Operator {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: ${symbol.toString().replaceAll('ComparativeSymbol.', '')}".indent(indent);
  }
...
class LogicalOp implements Operator {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: ${symbol.toString().replaceAll('LogicalSymbol.', '')}".indent(indent);
  }
...
class UnaryPrefixOp implements Operator {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: ${symbol.toString().replaceAll('UnaryPrefixSymbol.', '')}".indent(indent);
  }
...
class Value implements Literal, Expression {
  ...
  @override
  String toTree(int indent) {
    return "${this.runtimeType}: [${value.runtimeType}] ${value.toString()}".indent(indent);
  }
