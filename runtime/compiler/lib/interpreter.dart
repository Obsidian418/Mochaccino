...
class Interpreter {
  ...
  void checkBooleanOperand(dynamic op, Object? operand, Token token) {
    ...
    throw RuntimeError(
      Issue(
        ...
        description: "The operation '${token.lexeme}' requires that the operand '$operand' be of type [bool], but the type given is [${operand.runtimeType}].",
      ),
    );
  }
  ...
  void checkNumberOperand(dynamic op, Object? operand, Token token) {
    ...
    throw RuntimeError(
      Issue(
        IssueType.TypeError,
        IssueTitle.operationTypeError(token.lexeme, double, operand.runtimeType),
        lineNo: token.lineNo,
        start: token.start,
        offendingLine: sourceLines[token.lineNo],
        description: "The operation '${token.lexeme}' requires that the operand '$operand' be of type [num], but the type given is [${operand.runtimeType}].",
      ),
    );
  }

  void checkNumberOperands(dynamic op, Object? leftOperand, Object? rightOperand, Token token) {
    ...
    if (!(leftOperand is double)) {
      throw RuntimeError(
        Issue(
          ...
          start: token.start,
          offendingLine: sourceLines[token.lineNo],
          description: "The operation '${token.lexeme}' requires that the operand '$leftOperand' be of type [num], but the type given is [${leftOperand.runtimeType}].",
        ),
      );
    } else {
      throw RuntimeError(
        Issue(
          ...
          start: token.start,
          offendingLine: sourceLines[token.lineNo],
          description: "The operation '${token.lexeme}' requires that the operand '$rightOperand' be of type [num], but the type given is [${rightOperand.runtimeType}].",
        ),
      );
    }
  }
  ...
  Object? interpretUnary(UnaryExp expr) {
    ...
    switch (expr.unaryPrefix.symbol) {
      ...
      case UnaryPrefixSymbol.bang:
        checkBooleanOperand(expr.unaryPrefix.symbol, value, expr.unaryPrefix.op);
            ...
            if (left is double) {
              throw RuntimeError(
                Issue(
                  ...
                  description: "The operation '${expr.op.op.lexeme}' requires that the right operand '$right' be of type [num] because the left operand is of type [num], but the type given is [${right.runtimeType}].",
                ),
              );
              ...
              throw RuntimeError(
                Issue(
                  ...
                  start: expr.op.op.start,
                  offendingLine: sourceLines[expr.op.op.lineNo],
                  description: "The operation '${expr.op.op.lexeme}' requires that the right operand '$right' be of type [str] because the left operand is of type [str], but the type given is [${right.runtimeType}].",
                ),
              );
            }
          }
